...
**`/`** 表示上下排列关系，例如：`A / B` 表示 A 和 B 在同一层级，A 在上，B 在下。这是无排版的 HTML 文档中最常见的排列关系。  

**`+`** 表示左右排列关系，例如：`A + B` 表示 A 和 B 在同一层级，A 在左，B 在右。

**`[]`** 表示包含关系，例如：`A [ B / C ]` 表示 A 是 BC 的容器，B 和 C 在 A 的内部的同一层级，B 在上，C 在下。
